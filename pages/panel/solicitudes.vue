            ...
            <div class="col-12 np espacioTabla">
                <div class="table-responsive">
                    <table class="table table-hover text-center">
                        ...
                        <tbody>
                            <tr v-for="report in reports" :key="report.id" class="" v-bind:class="{'table-success': report.is_approved, 'table-warning': !report.is_approved}">
                            <th scope="row">{{report.id}}</th>
                            <td> {{report.message}} </td>
                            <td> {{report.user_id}} </td>
                             <td> {{report.listing_id}} </td>
                            <td> <button @click="getIDReclamo" class="btn btn-info" :value="report.id" :disabled="report.is_approved">Aceptar Reclamo</button> </td>
...
export default {
    ...
    created(){
        this.$axios.$get('https://dtodoaqui.xyz/api/claims/').then((response) => {
    ...
    methods:{
        ...
        this.idReclamo = e.target.value;
            this.$axios.$delete('https://dtodoaqui.xyz/api/claims/'+this.idReclamo)
                .then((response) => { 
                    location.reload();
        ...
        aceptarDenuncia(){
            ...
            this.$axios.$get('https://dtodoaqui.xyz/api/claims/'+this.idReclamo).then((response) => {
                ...
                this.$axios.$put('https://dtodoaqui.xyz/api/claims/'+this.idReclamo, solicitud,{
                    headers: {
